# Cybar - 智能鸡尾酒配方管理与精确ABV计算平台

![License](https://img.shields.io/badge/license-MIT-blue.svg)
![Node.js](https://img.shields.io/badge/node.js-v14+-green.svg)
![Express](https://img.shields.io/badge/express-~4.17.1-orange.svg)
![Feature](https://img.shields.io/badge/feature-ABV%20Calculator-brightgreen.svg)

## 📖 项目简介

Cybar是一个功能丰富的鸡尾酒配方管理和酒精度计算平台，集成了多种先进功能来为调酒爱好者和专业调酒师提供完整的数字化解决方案。该项目采用现代化的网页技术栈，提供直观友好的用户体验，并集成了AI智能推荐功能。

**🧮 专业酒精度计算器**：平台的核心工具之一，支持多成分复杂混合饮料的精确ABV计算，为专业调酒和配方开发提供可靠的数据支持。无论是简单的双成分混合还是复杂的多层次鸡尾酒，都能提供准确的酒精度计算结果。
...
- **多成分混合计算**：支持添加无限多种原料成分，动态计算最终ABV
- **实时计算反馈**：输入数据后即时显示计算结果，无需页面刷新
- **原料体积比例计算**：自动计算各种原料的最佳配比
- **专业调酒参考**：为专业调酒师和酒类爱好者提供精确的酒精度数据
...
酒精度计算器是Cybar平台的重要工具之一，专为调酒师和酒类爱好者设计，提供精确的酒精含量计算功能。

### 🎯 核心功能特点

#### 📊 多成分混合计算
- **无限成分添加**：支持添加任意数量的原料成分
- **动态成分管理**：可随时添加或删除成分，实时更新计算结果
- **精确度量支持**：支持毫升(ml)为单位的精确体积输入
- **酒精度范围**：支持0-100%的酒精度范围输入

#### ⚡ 实时计算引擎
- **即时反馈**：输入数据后立即显示计算结果
- **智能验证**：自动验证输入数据的合理性
- **错误提示**：友好的错误提示和输入指导
- **计算公式透明**：使用标准的加权平均算法确保结果准确性

#### 🎨 用户友好界面
- **直观操作**：简洁明了的表单设计，易于理解和使用
- **响应式布局**：支持桌面端和移动端完美适配
- **视觉反馈**：清晰的结果展示和状态提示
- **无障碍设计**：符合网页无障碍标准
...
```
最终ABV = (成分1体积 × 成分1酒精度 + 成分2体积 × 成分2酒精度 + ...) ÷ 总体积
```
...
```
最终ABV = (40×40% + 20×0% + 10×20%) ÷ (40+20+10)
        = (1600 + 0 + 200) ÷ 70
        = 1800 ÷ 70
        = 25.71%
```
...
#### 专业调酒师
- **配方开发**：开发新鸡尾酒配方时的酒精度控制
- **成本控制**：通过调整成分比例优化成本结构
- **标准化生产**：确保每次调制的酒精度一致性
- **法规合规**：满足不同地区对酒精度标注的法规要求
...
### 酒精度计算相关接口
- `POST /api/calculator/abv` - 计算多成分混合饮料的酒精度（支持复杂配方）
- `GET /api/calculator/help` - 获取酒精度计算公式和使用说明
...
│   ├── index.html                 # 酒精度计算器页面
│   └── calculator.js              # 计算逻辑实现（支持多成分ABV计算）
├── 🗂️ custom/                     # 自定义配方创建器
│   ├── index.html                 # 自定义调酒主页面
│   ├── custom.js                  # 主要交互逻辑
...
6. **酒精度计算器** (`/calculator/`)：多成分ABV精确计算、动态成分管理
...
#### 酒精度计算器模块
- **前端实现**：使用原生JavaScript实现动态表单管理
- **计算引擎**：采用精确的浮点运算，避免JavaScript精度丢失
- **数据验证**：客户端和服务器端双重数据验证
- **用户体验**：实时计算反馈，无需提交表单等待响应

#### 技术实现要点
```javascript
// 酒精度计算核心算法（来自实际代码）
function calculateABV(ingredientGroups) {
    let totalVolume = 0;
    let totalAlcoholVolume = 0;
    
    ingredientGroups.forEach(group => {
        const volume = parseFloat(group.querySelector('.ingredient-volume').value);
        const abv = parseFloat(group.querySelector('.ingredient-abv').value);
...
#### 功能特性实现
- **动态成分管理**：JavaScript动态添加/删除DOM元素
- **实时验证**：输入时边框颜色变化提示有效性
- **数据持久化**：计算结果实时显示，无需页面刷新
- **错误处理**：完善的输入验证和用户友好的错误提示
- **响应式更新**：成分编号自动重排，保持界面整洁
...
2. **输入第一个成分**
   - 在"成分 1 体积 (ml)"输入框中输入体积，例如：40
   - 在"成分 1 酒精度 (%)"输入框中输入酒精度，例如：40
...
3. 在成分2输入：体积10，酒精度18
4. 点击"计算 ABV"
5. 结果显示：最终酒精度 (ABV): **36.67** %

### ⚠️ 使用注意事项

- **体积单位**：统一使用毫升(ml)
- **酒精度范围**：输入0-100之间的数值
- **小数精度**：支持小数输入，结果保留2位小数
- **最少成分**：至少需要一个成分才能计算
- **实时验证**：无效输入会显示红色边框提示

---

*🍸 享受您的调酒之旅！准确的ABV计算让每一杯鸡尾酒都恰到好处。*
